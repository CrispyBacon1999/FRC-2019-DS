...
for (var i = 0; i < tuning_keys.length; i++) {
  ...
  let label = tuningElem
    .append(document.createElement("h4"))
    .text(tuning[key].label);
  let value = tuningElem
    .append(document.createElement("p"))
    .text(tuning[key].text);
...
function onValueChanged(key, value, isNew) {
  ...
  if (key.toLowerCase().includes("fms")) {
    console.log(key);
  }
  switch (key) {
    // Hatch NT Values
    case "/components/hatch/is_in_position":
      ...
      robot.hatch.is_in_position = value;
  ...
  if (tuning.keys().includes(key)) {
    tuning[key].val = value;
    document.getElementById("tuning-value-" + key).innerHTML = value;
  }
